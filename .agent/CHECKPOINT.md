...
P0-05 is active and not complete. Resume its existing attested session `20260923_055805_b43278` on locked `openai-codex / gpt-6-sol / high`. Revalidate the exact registration/interpreter, then rerun target shell, child ELF, shebang, native-init signal-forwarding/reaping, and clean shutdown tests against the locked image. Preserve exact evidence and clean only verified KEEMU-owned resources. The configured boundary must stop before any p0-06 worker launch.
...
Docker daemon access is available (`/var/run/docker.sock`, Engine 29.7.2), and the approved host `qemu-aarch64` binfmt registration is now independently verified. Full p0-05 target child/shebang execution, signal forwarding, orphan reaping, and clean shutdown evidence remain to be produced by the resumed locked worker. Port publishing, persistence, NFQUEUE, and later isolation/recovery work remain outside this subtask.
