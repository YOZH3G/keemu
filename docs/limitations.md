...
- MIPS and MIPSEL artifacts and execution are untested. Strict version-1 scenario/production-lock/persistent-environment input schemas exist, but IPK archive safety analysis, lifecycle scripts, scenario execution, matrices, NDM fixtures, event contracts, persistent registry mutation, filesystem diff, and full acceptance runs are not implemented.
