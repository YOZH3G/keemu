...
| A18 | PARTIAL | `tests/integration/p0_runtime_probe.py`; `fixtures/recipes/aarch64/keemu-init-p005.c`; `tests/unit/test_input_schemas.py` | Adopted daemon/short child, reap after exit, SIGTERM shutdown/0, differential keeper-death/1 vs PID 1 SIGINT/0, owner/run-id checked removal and independent absence in P0-05 report; version-1 environment input rejects unowned/mismatched resource metadata and unsafe registry paths | interruption/stale-resource recovery, atomic registry mutation, race-free file opens and Docker label reconciliation not proven; no full A18 PASS |
...
| A21 | PARTIAL | `tests/unit/test_reports.py`; `tests/unit/test_doctor.py`; `tests/unit/test_cli.py`; `tests/unit/test_input_schemas.py` | Schema-version-2 report model, strict metadata, schema parity, derived validation, write-once JSON/Markdown/operation-log bundles, failed-run preservation, and P0 report digests are documented. New version-1 input schemas have strict fields/versions, duplicate-key rejection, SHA-256 source/scenario binding, and generated-schema parity | full lifecycle/acceptance runs and complete runtime provenance remain absent |
